...
# сума за майстори = цена за час -> (30% от сумата за материали) * общ  брой часове
# сума за разходи = сума за материали + сума за майстори
# отпечатваме разходите

naylon_quantity = int(input())
paint_quantity = int(input())
diluent_quantity = int(input())
workers_hours = int(input())

sum_naylon = (naylon_quantity + 2) * 1.50
sum_paint = (paint_quantity + 0.1 * paint_quantity)* 14.50
sum_diluent = diluent_quantity * 5
sum_bags = 0.40
sum_materials = sum_naylon + sum_paint + sum_diluent + sum_bags
